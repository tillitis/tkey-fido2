...
void udp_send(int fd, uint8_t *buf, int size)
{
	...
	if (sendto(fd, buf, size, 0, (struct sockaddr *)&serveraddr,
		   sizeof(serveraddr)) < 0) {
...
// Receive 64 byte USB HID message, don't block, return size of packet, return 0
// if nothing
...
void usage(const char *cmd)
{
	...
	fprintf(stderr,
		"   -b      backing implementation: udp(default) or hidg\n");
	exit(1);
}
