...
#define check_buf(x)                                                           \
	assert(memcmp(state_prev_0xff->x, state_tmp_ptr->x,                    \
		      sizeof(state_tmp_ptr->x)) == 0);

bool migrate_from_FF_to_01(AuthenticatorState_0xFF *state_prev_0xff,
			   AuthenticatorState_0x01 *state_tmp_ptr)
{
	// Calculate PIN hash, and replace PIN raw storage with it; add version
	// to structure other ingredients do not change
	if (state_tmp_ptr->data_version != 0xFF)
		return false;

	static_assert(sizeof(AuthenticatorState_0xFF) <=
			  sizeof(AuthenticatorState_0x01),
		      "New state structure is smaller, than current one, which "
		      "is not handled");

	if (ctap_generate_rng(state_tmp_ptr->PIN_SALT,
			      sizeof(state_tmp_ptr->PIN_SALT)) != 1) {
		printf2(TAG_ERR, "Error, rng failed\n");
		return false;
	}
	if (state_prev_0xff->is_pin_set) {
		crypto_sha256_init();
		crypto_sha256_update(state_prev_0xff->pin_code,
				     state_prev_0xff->pin_code_length);
		...
		crypto_sha256_update(state_tmp_ptr->PIN_SALT,
				     sizeof(state_tmp_ptr->PIN_SALT));
...
void do_migration_if_required(AuthenticatorState *state_current)
{
	...
	// FIXME rework to raw buffers with fixed size to allow state structure
	// size decrease
	...
	if (state_current->data_version == 0xFF) {
		...
		bool success = migrate_from_FF_to_01(
		    (AuthenticatorState_0xFF *)&state_previous, &state_tmp);
		if (!success) {
		...
		dump_hex1(TAG_ERR, (void *)&state_previous,
			  sizeof(state_previous));
