...
int16_t bridge_u2f_to_extensions(uint8_t *_chal, uint8_t *_appid, uint8_t klen,
				 uint8_t *keyh)
{
	int8_t ret = 0;
	uint32_t count;
...
int16_t extend_u2f(APDU_HEADER *req, uint8_t *payload, uint32_t len)
{
	...
	if (req->ins == U2F_AUTHENTICATE) {
		if (req->p1 == U2F_AUTHENTICATE_CHECK) {
			...
			if (is_extension_request((uint8_t *)&auth->kh,
						 auth->khl)) // Pin requests
			{
						  ...
						  auth->khl)) // Pin requests
			{
				...
				dump_hex1(TAG_EXT, (uint8_t *)&auth->kh,
					  auth->khl);
				goto end;
			}
			rcode = bridge_u2f_to_extensions(auth->chal, auth->app,
							 auth->khl,
							 (uint8_t *)&auth->kh);
		}
