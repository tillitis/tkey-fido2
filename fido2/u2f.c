						      ...
						      // do anything...
	{
		...
		case U2F_REGISTER:
			...
			if (len != 64) {
				...
				timestamp();
				rcode = u2f_register(
				    (struct u2f_register_request *)payload);
...
int8_t u2f_response_writeback(const uint8_t *buf, uint16_t len)
{
	if ((_u2f_resp->length + len) > _u2f_resp->data_size) {
		printf2(TAG_ERR,
			"Not enough space for U2F response, writeback\n");
		exit(1);
	}
...
static void u2f_make_auth_tag(struct u2f_key_handle *kh, uint8_t *appid,
			      uint8_t *tag)
{
	uint8_t hashbuf[32];
		       ...
		       uint8_t *pubkey)
{
	...
	crypto_ecc256_derive_public_key((uint8_t *)kh, U2F_KEY_HANDLE_SIZE,
					pubkey, pubkey + 32);
	return 0;
}
